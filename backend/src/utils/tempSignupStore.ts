...
export async function setTempSignupData(
  username: string,
  email: string,
  password: string,
  ...
  const [rows] = await pool.execute<ResultSetHeader>(
    `INSERT INTO temp_signup_data (username, email, password, isAdmin, otp)
   VALUES (?, ?, ?, ?, ?)
...
export async function getTempSignupData(email: string): Promise<RowDataPacket> {
  const [rows] = await pool.execute<RowDataPacket[]>(
    "SELECT * FROM temp_signup_data where email = ?",
    [email],
  );
...
export async function deleteTempSignupData(email: string): Promise<number> {
  const [rows] = await pool.execute<ResultSetHeader>(
    "DELETE FROM temp_signup_data where email = ?",
    [email],
  );
